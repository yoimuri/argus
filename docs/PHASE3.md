...
**Status:** 🟡 IN PROGRESS — Sprint 3a.1 is code-complete, pending Clint's deploy + live
verification. Sprints 3a.2–3b are still ⏳ not started. Every checkbox is ⏳ until the
sprint that owns it is code-complete (🟡) and then live-verified (✅) per the marks rule
in `CLAUDE.md`. This file is the execution plan, not a status claim.
...
> **How to read this doc (for the session executing it, likely Sonnet 5):** Phase 3 is
> split into **3a** (internal multi-agent pipeline + Debug Diary + observability — all on
> the user's own uploaded PDFs) and **3b** (Web Scout / live web search, added only after
> 3a is verified live). Build and verify 3a end-to-end before touching 3b. Each sub-sprint
> ends with a live verification gate; do not flip a ⏳ to ✅ until its gate has run against
> the deployed Render + Vercel app and the result is recorded in
> `docs/ADVERSARIAL-TESTS.md` / `CONTINUITY.md`.

## Execution ground rules (Clint's directive, 2026-07-07)

**Follow this plan strictly. Deviate only when you hit a real problem** — the code contradicts
an assumption here, a library behaves differently than documented, or a step is genuinely
impossible as written. Cosmetic preference is not a reason to deviate.

When you do deviate: **stop, don't silently improvise.** State the problem, propose the fix, and
either (a) flag it to Clint and wait if it changes scope/architecture, or (b) if it's a small
mechanical correction, make it and record it in `CONTINUITY.md` + the relevant ADR in the same
turn. Every deviation must leave a paper trail — that is how this project avoids the doc-vs-code
drift it was built to fight.

Hard boundaries that override any step here: **Claude never commits, pushes, or touches live
platforms** (Supabase SQL, Render/Vercel env + deploys). Those are Clint's manual steps —
prepare the exact SQL/env/commands, explain them, and stop. Build one sprint, hand off its manual
step, wait for the live-verify result, then start the next.
...
and **how you debug a 2am failure without re-running anything** (the Debug Diary — the blueprint
calls it "the differentiator feature"). Keeping that vision intact is the point of this plan.
...
each sprint lands, update `BLUEPRINT.md`, `PHASE3.md` (this file), `CONTINUITY.md`, and
`ADVERSARIAL-TESTS.md` **in the same turn**, and only mark ✅ after a live check.
...
One migration (007), needed starting at Sprint 3a.2. **Reminder (`CLAUDE.md` platform rule):**
migrations do NOT auto-run — Clint pastes them into the Supabase SQL editor by hand. New tables
are NOT auto-exposed (grants are explicit, see migration 002) and RLS is auto-force-enabled by
...
- The pinned criterion: send `"summarize for me"` (and `"what's the gist"`) against a real
  ...
  before/after in `CONTINUITY.md` and close the `BACKLOG.md` Item 1 row.
...
**Build:**
- Run **migration 007** (Clint's manual step — below).
...
## Manual platform steps (all Clint's — prepare, explain, stop)

Per `CLAUDE.md`, everything touching live systems is Clint's. Consolidated:

1. **Migration 007** — paste into Supabase SQL editor by hand; verify `execution_steps` exists
   with its RLS policy + grants. (Sprint 3a.2)
2. **Langfuse Cloud** — create account + project, copy keys, set `LANGFUSE_*` env on Render.
   (Sprint 3a.4)
3. **Tavily** — create account, set `TAVILY_API_KEY` on Render. (Phase 3b only)
4. **Deploys** — every sprint ends with a git push (Claude never commits/pushes); Render +
   Vercel rebuild on push. "Deployed" = Clint re-checked the live URLs. Render free tier: first
   hit after idle is 30–60 s, not a bug.
...
recorded (pass or fail) in `docs/ADVERSARIAL-TESTS.md` / `CONTINUITY.md`:

1. **Vague-query fix (the pinned one):** `"summarize for me"` returns a real, representative
   answer, not a "no information" refusal. (`BACKLOG.md` Item 1 closes — Sprint 3a.1.)
...
## Docs to keep in sync (same turn as the code, per CLAUDE.md)
...
- **`ADVERSARIAL-TESTS.md`** — new gates for the StepWriter chaos test, the loop cap, the
  Langfuse-down degradation; the 3b web-injection gates.
- **`CONTINUITY.md`** — the running log, updated each session.
