...
| # | Class | Input / Action | Expected | Status |
|---|---|---|---|---|
| GATE-18 | Authorization, session-list isolation | As user A, call `GET /research` after both A and B have run research sessions | The response contains only A's own sessions — B's session ids never appear, regardless of how many B has | 🟡 Partial, live 2026-07-09: endpoint smoke-tested with `test@argus.dev` — `GET /research?limit=5` returned exactly 5 own sessions, correct shape (`id, collection_id, query, status, created_at`, no `report` field), correct `created_at` desc order. Cross-user isolation (a second account never seeing these rows) not yet exercised — needs a second test account |
