...
| GATE-25 | Resilience, cancel is real | Cancel a research query mid-run (button or navigate away); separately cancel an upload mid-processing | The session lands on `status: "cancelled"`, not stuck at `"running"` forever, and no report gets written after; the cancelled upload's document does NOT appear (full delete — no phantom), re-uploading yields exactly ONE document, and a query against that collection finds nothing from the cancelled upload | **❌ FAILED first cut, live 2026-07-10; reworked, awaiting re-test.** The 2026-07-09 implementation relied on `asyncio.CancelledError`, which uvicorn does NOT raise into a running handler on client disconnect — confirmed live: a "cancelled" upload completed anyway and re-uploading doubled the doc, and research kept running after navigate-away. **This is exactly the unverified assumption the previous 🟡 flagged, now proven false — the discipline worked, the code did not.** Reworked 2026-07-10 to cooperative cancellation: `/research` races the graph task against a `request.is_disconnected()` poll and cancels the task itself; upload polls `is_disconnected()` and fully deletes the document (row + chunks) on disconnect. Re-test is load-bearing (`docs/PHASE4.md` Sprint 4.3 "Verify live" step 2) — if `is_disconnected()` also doesn't fire on Render, the fallback is an explicit `/research/{id}/cancel` flag endpoint. Do NOT mark ✅ without the live proof. |
