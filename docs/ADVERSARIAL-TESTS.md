...
GATE-25 (cancel) is exercisable now, no second account needed. Run each against the live
deployed app once its dependency ships; record pass/fail below same as every prior suite.
...
| GATE-25 | Resilience, cancel is real | Cancel a research query mid-run (button or navigate away); separately cancel an upload mid-processing | The session lands on `status: "cancelled"`, not stuck at `"running"` forever, and no report gets written after; the document lands on `status: "failed"`, and a query against that collection finds nothing from its partial chunks (proof `_delete_partial_chunks` actually ran, not just that the row got marked failed) | 🟡 Code-complete 2026-07-09 (Sprint 4.3, D15) — `asyncio.CancelledError` handling added to both `/research` and `/collections/{id}/documents` in `backend/main.py`, frontend `AbortController` wired through `api.ts`'s existing `signal` support. **This is the sprint's one unverified assumption, stated plainly**: the whole mechanism depends on Starlette/uvicorn actually delivering a client disconnect as `CancelledError` inside the running handler coroutine — standard behavior for this stack, but never confirmed against this specific app before today. Not provable by reading source; needs the live test in `docs/PHASE4.md` Sprint 4.3's "Verify live" step 2 |
