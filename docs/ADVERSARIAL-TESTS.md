...
| GATE-28 | Report generation: metering, isolation, honesty (ADR-022) | (a) Set a test account's `max_reports_per_day` to 1 in Studio, generate one report (succeeds), then start a second; (b) after hitting the cap, DELETE the generated report AND its source collection, then try again; (c) as user A, open `/dashboard/reports/<B's report id>` and call `GET /reports/{B's id}` + `/docx` directly; (d) cancel a generation mid-run; (e) open a completed report: check the disclaimer, then download the `.docx` and the PDF; (f) on a completed research session, click "Generate report from this answer"; (g) generate one report in each mode — Quick draft and Full report | (a) The second attempt returns a friendly **429** before any Groq/Tavily work; (b) still 429 — deleting the report/collection refunds nothing (usage_events is the count source); (c) "Report not found" in the UI and 404 from both endpoints, indistinguishable from a nonexistent id; (d) the row lands `status='cancelled'`, no content ever appears on it, and the generation stops; (e) the needs-proofreading disclaimer is visible in the preview, inside the .docx (under the title), and in the printed PDF — and the .docx opens cleanly in Word/LibreOffice with headings and lists intact; (f) a report is produced from the session's existing answer (metered as one report) without re-processing the documents. (g) Performance expectation (ADR-022 rev. 3): a **Quick draft on a warm dyno completes in well under 30s** (one model call; a cold dyno adds its 30–60s wake-up, which the progress copy says); a **Full report** is visibly paced — the progress bar's stage text advances ("Reading documents (k/N)…", "Writing the report…"), minutes on a large collection, never silently hung. A failed run shows a specific reason on the report page (`error_detail`, migration 018), not a bare "Generation failed"; `/status/breakers` shows the separate `groq_report` card, and a failing report run must NOT degrade a concurrent Ask (quota + breaker isolation) | 🟡 Code-complete 2026-07-13 (revised: meter-paced engine + isolation + session source + error_detail; 2026-07-14: Quick/Full modes + progress + real PDF download), not run live. Needs migrations 017 + 018 + 019 pasted first; (c) needs the second test account |
