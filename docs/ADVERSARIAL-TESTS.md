...
| GATE-28 | Report generation: metering, isolation, honesty (ADR-022) | (a) Set a test account's `max_reports_per_day` to 1 in Studio, generate one report (succeeds), then start a second; (b) after hitting the cap, DELETE the generated report AND its source collection, then try again; (c) as user A, open `/dashboard/reports/<B's report id>` and call `GET /reports/{B's id}` + `/docx` directly; (d) cancel a generation mid-run; (e) open a completed report: check the disclaimer, then download the `.docx` and Save-as-PDF | (a) The second attempt returns a friendly **429** before any Groq/Tavily work; (b) still 429 — deleting the report/collection refunds nothing (usage_events is the count source); (c) "Report not found" in the UI and 404 from both endpoints, indistinguishable from a nonexistent id; (d) the row lands `status='cancelled'`, no content ever appears on it, and the generation stops (Render logs show the task exiting between model calls); (e) the needs-proofreading disclaimer is visible in the preview, inside the .docx (under the title), and in the printed PDF — and the .docx opens cleanly in Word/LibreOffice with headings and lists intact | 🟡 Code-complete 2026-07-13, not run live. Needs migration 017 pasted first; (c) needs the second test account |
