...
| 2026-07-11 | **Simulated subscription tier** — specific users get automatically raised (and monthly-reset) usage limits, as if subscribed; the "subscribe" mechanism itself stays locked/disabled since this is not a profit-first project (it demonstrates the capability, it doesn't charge anyone). Today's reality: per-user raises already work (edit that user's `usage_limits` row in Studio; after migration 012, `user_profiles` maps uuid→email so you know whose row it is), and the research cap is already a rolling 24h window needing no reset. What does NOT exist: any automatic tier logic, monthly reset for collections/documents, or an admin surface to manage it — that's role/admin machinery, i.e. **Phase 4b** scope. | parked for 4b — not this phase, per the locked read-only-no-roles decision |
