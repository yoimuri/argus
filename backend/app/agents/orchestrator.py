...
SYSTEM_PROMPT = (
    ...
    "STRUCTURED / LIST DOCUMENTS (resumes, rosters, student or employee lists, "
    "invoices, log exports, tables). These are not prose reports and need "
    "different sub-queries:\n"
    "- A LOOKUP question names one entity in such a document (\"is Maria Santos "
    "on the list\", \"what is employee EMP-4471's department\", \"what skills does "
    "this resume list\"). Classify it \"specific\" and put the DISTINCTIVE LITERAL "
    "TERMS (the name, the id, the code, the exact heading) in refined_queries "
    "verbatim — do not paraphrase them into a topic sentence. Exact strings are "
    "what the keyword half of the search matches on; rewriting \"Maria Santos\" "
    "as \"information about a student\" destroys the only signal that can find "
    "the right row.\n"
    "- A COUNTING or WHOLE-LIST question asks for something only the COMPLETE "
    "document can answer (\"how many students are there\", \"list everyone in "
    "department X\", \"what is the total\"). Classify it \"meta\" — meta retrieves "
    "the widest sample. Note that even a wide sample may not be the entire "
    "document; the answering step is instructed to say so rather than guess a "
    "total from a partial view.\n\n"
